...
Shader "URP/Anime/AnimeBody"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half3 _SkinColor;
			half3 _SkinShadeColor;
			half3 _ToonProximityAmbience;
			half _Dirt;
			half3 _FingerNailColor;
			half3 _ToeNailColor;
			half _FogDensity;
			half3 _FogColor;
			...
			v2f vert (appdata v)
			{
				...
				o.uv = v.uv;
				o.fogFactor = exp(-_FogDensity * distance(_WorldSpaceCameraPos.xyz, o.worldPos));
				return o;
			}
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				// Calculs d'éclairage
				half ndotl = max(0.0, dot(i.worldNorm, -lightDir));
				...
				// Appliquez le brouillard
				half fogFactor = ComputeFogFactor(i.pos); // Assurez-vous que ComputeFogFactor est défini ou disponible
